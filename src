...
const STORAGE_KEY = "bidarena-entries-v1";
...
export default function App() {
  ...
  const [dethroned, setDethroned] = useState(null);
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        setEntries(sorted);
        prevTopRef.current = sorted[0]?.id || null;
      } else {
        const sorted = sortEntries(STARTER_ENTRIES);
        setEntries(sorted);
        prevTopRef.current = sorted[0]?.id || null;
      }
    } catch (e) {
  ...
  return (
    <div style={styles.page}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Bebas+Neue&family=JetBrains+Mono:wght@400;500;600;700&family=Inter:wght@400;500;600;700&display=swap');
      ...
      <div className="oa-root" style={{ maxWidth: 480, margin: "0 auto", padding: "20px 14px 40px" }}>
        ...
        <p style={{ textAlign: "center", color: "#4B5063", fontSize: 11.5, marginTop: 24, lineHeight: 1.6 }}>
          Demo prototype — inspired by outbid.lol. No real payments are processed;<br />amounts are simulated and saved only in your own browser.
        </p>
      </div>
